        ...
        <div class="flex flex-col items-center justify-center">
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <div class="text-center mb-8">
                        <Button label="일반사용자" class="w-full" as="router-link" to="/"></Button>
                        <Button label="기관담당자" class="w-full" as="router-link" to="/"></Button>
                        <Button label="기관관리자" class="w-full" as="router-link" to="/"></Button>
                    </div>
                    <div>
                        <label for="user_name" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">이름</label>
                        <InputText id="user_name" type="text" placeholder="성함을 입력해주세요" class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="user_id" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">아이디</label>
                        <InputText id="user_id" type="text" placeholder="아이디를 입력해주세요" class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="user_pw" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">비밀번호</label>
                        <Password id="user_pw" v-model="password" placeholder="Password" :toggleMask="true" class="mb-4" fluid :feedback="false"></Password>

                        <label for="user_pwd" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">비밀번호 확인</label>
                        <Password id="user_pwd" v-model="password" placeholder="Password" :toggleMask="true" class="mb-4" fluid :feedback="false"></Password>

                        <label for="user_email" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">이메일</label>
                        <InputText id="user_email" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="user_account" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">인증번호</label>
                        <InputText id="user_account" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="tel" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">연락처</label>
                        <InputText id="tel" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="address" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">주소</label>
                        <InputText id="address" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-8" v-model="email" />

                        <label for="institution" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">기관</label>
                        <InputText id="institution" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-8" v-model="email" />
